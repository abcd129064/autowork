...
class AddRecordDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        form.addRow("球桌名:", self._edit_name)
    ...
    def _on_ok(self):
        if not self._edit_name.text().strip():
            self._edit_name.setPlaceholderText("球桌名不能为空")
...
class TablePanelWindow(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self._hidden_cols: set = set() # 隐藏列索引
    ...
    def _build_col_menu(self):
        ...
        for i, (_, title, _) in enumerate(COLUMNS):
            ...
            # addWidget 以 widget.size() 作为菜单项尺寸提示，
    ...
    def _copy_selected(self):
        ...
        # 若当前有打开的只读编辑器且光标选中了部分文本，优先复制该部分
        editor = self._table.currentEditor()
