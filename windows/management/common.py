...
class TextDragSelectTableMixin:
    """单元格文本字符级拖选复制 Mixin（2026-09-24 自健康页 _HealthCopyTable 抽出）

    QTableWidget 默认只能整格/整行选择，无法像文本编辑器那样拖动鼠标
    选中部分文字。本 Mixin 在单元格文本上实现字符级拖选：按住左键拖动
    即出蓝色选区高亮，松开后 Ctrl+C 或右键「复制选中」写入系统剪贴板；
    未拖选时 Ctrl+C / 右键复制整行（回退文本见 row_text）。
    选区几何与 delegate 绘制文本完全同源（同字体 + 固定内缩文本矩形），
    保证高亮与文字逐字对齐。

    用法::

        class MyTable(TextDragSelectTableMixin, TableWidget):
            _TEXT_COLS = (1, 2, 3)   # 参与拖选/复制的文本列

    可覆盖钩子：
        _on_text_double_clicked(row, col) -> bool
            双击钩子：返回 True 表示双击已被子类处理（如文件列表双击
            打开图片），不再走默认的「双击选中该格全部文本」；
        row_text(row) -> str
            Ctrl+C / 右键「复制整行」的回退文本（默认按 _TEXT_COLS
            制表符分列拼接，便于贴到 Excel/表格）。

    ⚠️ 文本矩形用固定内缩几何近似（_TEXT_INSET），禁止在事件回调里调
    style().subElementRect 动态测量——offscreen 平台下该调用会污染状态
    导致后续 QTableView 原生鼠标/绘制路径访问违例崩溃（delegate 内部
    调用安全）。
    """

    # 文本列（子类必须覆盖；首列若是勾选框/控件列勿含）
    _TEXT_COLS: tuple = ()

    # 单元格文本矩形相对单元格矩形的左右内缩（px）。
    # 实测（Windows 风格 + qfluentwidgets TableItemDelegate，左对齐）：
    # visualRect x+16 起绘、右侧对称内缩 16。
    _TEXT_INSET = (16, 16)
    ...
    def row_text(self, row):
        """整行回退文本（制表符分列，便于粘贴到 Excel/表格）；子类可覆盖"""
    ...
    def _on_text_double_clicked(self, row, col) -> bool:
        """双击钩子：默认不处理（返回 False 走「选中该格全部文本」）"""
        return False
    ...
    def mouseDoubleClickEvent(self, event):
        """双击：子类钩子优先，否则选中该格全部文本（贴近文本编辑器习惯）"""
        if event.button() == Qt.MouseButton.LeftButton:
            hit = self._index_at_text_col(event.position().toPoint())
            if hit is not None:
                row, col, _ = hit
                if self._on_text_double_clicked(row, col):
                    event.accept()
                    return
