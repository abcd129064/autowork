...
# ==================== MySQL 自动推送（API 同步后静默触发） ====================

_mysql_auto_worker = None


def _trigger_auto_mysql_sync():
    """检查 auto_sync 配置，若开启则静默推送本地数据到 MySQL

    全局单 worker：新触发时若上一个还在跑则忽略，避免并发推送。
    """
    ...
    if _mysql_auto_worker and _mysql_auto_worker.isRunning():
        return
    _mysql_auto_worker = MysqlSyncWorker()
    _mysql_auto_worker.start()
...
class AdminSettingsPage(QWidget):
    ...
    def _init_ui(self):
        ...
        # MySQL 远程同步卡片（独立组件，售后面板复用同款配置/测试/同步）：同步 5 张运维业务表
    ...
    def _load_current(self):
        ...
        # MySQL 同步配置
        mysql_cfg = settings.get("mysql_sync", {})
        self._mysql_card.load(mysql_cfg)
    ...
    def _on_save(self):
        ...
        try:
            _save_settings({
                ...
                # 取卡片表单当前值（含密码，加密落盘）
                "mysql_sync": self._mysql_card.collect_cfg(),
            })
    ...
    # MySQL 配置/测试/同步已收敛到 MysqlSyncCard 组件（windows/mysql_sync_card.py），
    # 与售后面板复用同一套逻辑：测试/同步直接用表单配置，同步前自动落盘生效。
