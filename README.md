...
### 台账面板（主界面「写入台账」按钮打开）
- **会话预填**：打开面板自动带入当前球桌会话（球房/视频名/帧数/署名），表单确认后提交
...
- **设置**：默认署名配置 + 数据库设置（复用 MysqlSyncCard，同步范围为台账记录）
...
| 本地数据库 | SQLite3（球桌/设备状态/售后/台账记录缓存） |
...
│   ├── ledger_db.py           #   台账记录数据层（分类筛选统计、署名统计、导出）
...
│   ├── ledger_panel.py        #   台账面板（填写录入/记录统计/设置三页，re-export shim）
│   ├── ledger/                #   台账面板拆分包（表单/页面/设置/窗口）
│   ├── mysql_sync_card.py     #   MySQL 同步配置卡片（运维/售后/台账面板共用）
