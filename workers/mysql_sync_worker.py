...
class MysqlSyncWorker(QThread):
    ...
    def __init__(self, table_name: str = None, cfg: dict = None, parent=None):
        """table_name 为空时推送全部表，否则只推指定表

        cfg 为连接配置 dict；缺省时从 settings.json 读取（面板传表单
        配置可做到测试通过后无需先保存即可同步）。
        """
    ...
    def run(self):
        try:
            ...
            if self._table_name == "aftersale_records":
                # 售后记录：业务键去重 upsert（多用户共享库，不能全量覆盖）
                ok, msg, count = mysql_sync.push_aftersale(
                    progress_cb=self.progress.emit, cfg=self._cfg)
